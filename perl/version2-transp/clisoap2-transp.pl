#!/usr/bin/perl
# clisoap2.pl
# Ejecutar: perl clisoap2.pl
# URL:      http://localhost:8000/?n=10  ->  diez
#
# 1. Consume el servicio web SOAP para obtener el número en inglés
# 2. Traduce al ESPAÑOL usando MyMemory API (gratuita, sin API key)
#
# Módulos requeridos (instalar con apt):
#   apt install libwww-perl libxml-simple-perl libjson-perl

use strict;
use warnings;
use IO::Socket::INET;
use LWP::UserAgent;
use XML::Simple;
use URI::Escape qw(uri_escape);

my $SOAP_URL      = 'https://www.dataaccess.com/webservicesserver/NumberConversion.wso';
my $TRANSLATE_URL = 'https://api.mymemory.translated.net/get';
my $PORT          = 8000;

# ── SOAP ──────────────────────────────────────────────────────────────────────

sub build_envelope {
    my ($numero) = @_;
    return <<XML;
<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <NumberToWords xmlns="http://www.dataaccess.com/webservicesserver/">
      <ubiNum>$numero</ubiNum>
    </NumberToWords>
  </soap:Body>
</soap:Envelope>
XML
}

sub find_value {
    my ($node, $target) = @_;
    return undef unless ref($node) eq 'HASH';
    for my $key (keys %$node) {
        my $local = $key;
        $local =~ s/^[^:]+://;
        return $node->{$key} if $local eq $target;
        my $found = find_value($node->{$key}, $target);
        return $found if defined $found;
    }
    return undef;
}

sub number_to_words {
    my ($numero) = @_;
    my $envelope = build_envelope($numero);
    my $ua       = LWP::UserAgent->new(timeout => 10);

    my $response = $ua->post(
        $SOAP_URL,
        Content_Type => 'text/xml; charset=utf-8',
        SOAPAction   => 'http://www.dataaccess.com/webservicesserver/NumberToWords',
        Content      => $envelope,
    );

    die "Error HTTP SOAP: " . $response->status_line unless $response->is_success;

    my $xml  = XML::Simple->new(ForceArray => 0, KeyAttr => []);
    my $data = $xml->XMLin($response->content);
    my $body = $data->{'soap:Body'} // $data->{'Body'};
    die "No se encontró soap:Body" unless $body;

    my $result = find_value($body, 'NumberToWordsResult');
    die "No se encontró NumberToWordsResult" unless defined $result;

    return $result;
}

# ── Traducción (MyMemory API) ──────────────────────────────────────────────────

sub translate_to_spanish {
    my ($text) = @_;

    # uri_escape codifica correctamente el | como %7C
    my $q        = uri_escape($text);
    my $langpair = uri_escape('en|es');    # -> en%7Ces
    my $url      = "$TRANSLATE_URL?q=$q&langpair=$langpair";

    my $ua       = LWP::UserAgent->new(timeout => 10);
    my $response = $ua->get($url, Accept => 'application/json');

    die "Error HTTP traducción: " . $response->status_line unless $response->is_success;

    my $json = $response->content;

    # Extraer translatedText sin módulo JSON externo
    # Respuesta: {"responseData":{"translatedText":"diez",...},...}
    if ($json =~ /"translatedText"\s*:\s*"([^"]+)"/) {
        return $1;
    }
    die "No se encontró translatedText en: $json";
}

# ── Servidor HTTP ──────────────────────────────────────────────────────────────

sub get_param {
    my ($query, $name) = @_;
    return undef unless defined $query;
    for my $pair (split /&/, $query) {
        my ($k, $v) = split /=/, $pair, 2;
        return $v if defined $k && $k eq $name;
    }
    return undef;
}

sub send_response {
    my ($client, $status, $text, $body) = @_;
    print $client "HTTP/1.1 $status $text\r\n";
    print $client "Content-Type: text/plain; charset=utf-8\r\n";
    print $client "Content-Length: " . length($body) . "\r\n";
    print $client "Connection: close\r\n";
    print $client "\r\n";
    print $client $body;
}

my $server = IO::Socket::INET->new(
    LocalPort => $PORT,
    Type      => SOCK_STREAM,
    Reuse     => 1,
    Listen    => 10,
) or die "No se pudo iniciar el servidor en el puerto $PORT: $!";

print "[clisoap2] Servidor corriendo en http://localhost:$PORT\n";
print "Prueba: http://localhost:$PORT/?n=10\n";

while (my $client = $server->accept()) {
    $client->autoflush(1);

    my $request_line = <$client>;
    chomp $request_line;

    while (my $line = <$client>) {
        last if $line =~ /^\r?\n$/;
    }

    if ($request_line =~ m{GET /favicon\.ico}) {
        print $client "HTTP/1.1 204 No Content\r\nConnection: close\r\n\r\n";
        close $client;
        next;
    }

    my ($query) = $request_line =~ m{GET /\?(.+) HTTP}i;
    my $n = get_param($query, 'n');

    my ($status_code, $status_text, $body);

    if (!defined $n || $n !~ /^-?\d+$/) {
        ($status_code, $status_text, $body) = (400, 'Bad Request', 'Uso: /?n=<numero>   Ejemplo: /?n=10');
    } else {
        eval {
            my $en_ingles = number_to_words($n);
            $body = translate_to_spanish($en_ingles);
            ($status_code, $status_text) = (200, 'OK');
        };
        if ($@) {
            ($status_code, $status_text, $body) = (500, 'Internal Server Error', "Error: $@");
        }
    }

    send_response($client, $status_code, $status_text, $body);
    close $client;
}